...
export function saveStage(_fetch1=fetch, _fetch2=fetch) {
    return async function sideEffect(dispatch, getState) {
        ...
        if (textAnswer === "" && booleanAnswer === "" && mcAnswer === "") {
            ...
            if (index === 0) {
                // first save
                const url1 = `https://process-manager-resource-dd3ea938a14b.herokuapp.com/saveFirstFinishedStage?question=${question}&stageType=${stageType}` +
                ...
                // regular save
                const url2 = `https://process-manager-resource-dd3ea938a14b.herokuapp.com/saveFinishedStage?token=${token}&question=${question}&stageType=${stageType}` +
                    `&textAnswer=${textAnswer}&booleanAnswer=${booleanAnswer}&mcAnswer=${mcAnswer}&stageOrder=${stageOrder}`
...
export function saveProcess(_fetch=fetch) {
    return async function sideEffect(dispatch, getState) {
        ...
        const url = `https://process-manager-resource-dd3ea938a14b.herokuapp.com/saveFinishedProcess?token=${token}&title=${title}`
        const response = await _fetch(url)
        if (response.ok) {
